"""This class comes from the https://github.com/flaviostutz repo, specifically from:
https://github.com/flaviostutz/sentinelloader/blob/master/sentinelloader/sentinel2loader.py

This modules allows to download Sentinel 2 data by supplying the user info and password. It was not
possible to use this model directly due to some segmentation faults in installing the module (possibly
due to the fact that I'm using an m1 mac machine) so I have reported the code here.
"""
...
class Sentinel2Loader:
    ...
    def getProductBandTiles(self, geoPolygon, bandName, resolution, dateReference):
        ...
        for index, sp in products_df.loc[selectedTiles].iterrows():
            url = (
                "https://apihub.copernicus.eu/apihub/odata/v1/Products('%s')/Nodes('%s.SAFE')/Nodes('MTD_MSIL%s.xml')/$value"
            ...
            if not self.cacheTilesData or not os.path.isfile(downloadFilename):
                ...
                if productLevel == "2A":
                    url = (
                        "https://apihub.copernicus.eu/apihub/odata/v1/Products('%s')/Nodes('%s.SAFE')/Nodes('GRANULE')/Nodes('%s')/Nodes('IMG_DATA')/Nodes('R%s')/Nodes('%s.jp2')/$value"
                        % (
                            sp["uuid"],
                            sp["title"],
                            m.group(1),
                    ...
                    url = (
                        "https://apihub.copernicus.eu/apihub/odata/v1/Products('%s')/Nodes('%s.SAFE')/Nodes('GRANULE')/Nodes('%s')/Nodes('IMG_DATA')/Nodes('%s.jp2')/$value"
    ...
    def cropRegion(self, geoPolygon, sourceGeoTiffs):
        ...
        logger.debug(
            "Combining tiles into a single image. sources=%s tmpfile=%s" % (source_tiles, tmp_file)
        )
        os.system(
                ...
                if minVisibleLand > 0:
                    try:
                        ...
                        if visibleLandRatio < minVisibleLand:
                            raise Exception(
                                "Too few land shown in image. visible ratio=%s" % visibleLandRatio
                            )
                        else:
                            logger.debug(
